...
if(importCode){
 ...
 save.onclick=()=>{let name=data.name;while(collections.some(c=>c.name===name))name+=' (copy)';const ids=[];for(const a of data.items){ids.push(a.id);if(!liked.has(a.id))liked.set(a.id,{...a,src:a.id[0]==='v'?'how':'wiki',img:a.img||'',url:`https://${a.lang}.${a.id[0]==='v'?'wikivoyage':'wikipedia'}.org/?curid=${a.id.slice(1)}`});}collections.push({name,ids});saveCollections();saveLiked();activeCollection=collections.length-1;renderLikedList();dlg.close();toast('Collection saved');};
 cancel.onclick=()=>dlg.close();const actions=document.createElement('div');actions.className='collection-dialog-actions';actions.append(save,cancel);dlg.append(heading,note,actions);document.body.append(dlg);dlg.showModal();
